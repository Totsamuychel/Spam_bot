...
class TelegramBot:
    ...
    async def account_management_menu(self):
        ...
        while True:
            ...
            print("4. Показать все аккаунты с подробной информацией")
            print("5. 🧹 Очистить память и сбросить лимиты")
            print("6. 🔄 Проверить и переподключить аккаунты")
            print("7. 📅 Синхронизировать планировщик")
            ...
            if choice == '1':
                ...
                # Показать все аккаунты с подробной информацией
                print("\n🔍 ПРОВЕРКА ВСЕХ АККАУНТОВ")
                print("="*60)
                accounts = await self.auth_manager.list_all_accounts()
                if not accounts:
                    print("📭 Нет добавленных аккаунтов")
                else:
                    print(f"Найдено {len(accounts)} аккаунтов:")
                    for account in accounts:
                        self.auth_manager.print_account_info(account)
